...
import { ScrollView, StyleSheet, View, SafeAreaView, Alert, Text, Button, Linking, Pressable, ActivityIndicator } from 'react-native';
    ...
    try {
      ...
      if (cachedResponse && JSON.parse(cachedResponse).email) {
        ...
        if (parsedCachedResponse['active']=="TRUE"){
          setuser_id(parsedCachedResponse['user']);
        ...
        [
          ...
          },
          {
            ...
            onPress: async () => {
              const cachedResponse = await AsyncStorage.getItem('aab_prahari_register_cache');
              const parsedCachedResponse = JSON.parse(cachedResponse);
              const active_user=parsedCachedResponse["email"];
              const deleteData = new FormData();
              deleteData.append('email', active_user);
              try{
              const response = await axios.post('https://jalsuraksha.iitd.ac.in/barapullah/aab_prahari/deactivateappuser_ios.php', deleteData, {
                headers: {
                  'Content-Type': 'multipart/form-data',
                },
              });
            Alert.alert("Account deleted successfully.","",
              [
                ...
                Alert.alert("Error Deleting Account","Please contact us on swmmiitd@gmail.com",
                  [
                    {
                      text: "Ok",
                      onPress: async () => {
    ...
    <SafeAreaView style={styles.body}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.userspecs}>
          ...
          <Button title="Delete Account" onPress={SignOut} color="red" />
        </View>
